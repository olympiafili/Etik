class Paraggelia < ActiveRecord::Base
    ...
    CSV.generate(options) do |csv|
      ...
      self.orders.each do |order|
        ...
        #SURCHARGE_COLOR
        csv << ["SURCHARGE_COLOR_IN", order.surcharge_color, order.main_color_id]
        csv << ["SURCHARGE_COLOR_OUT", order.surcharge_color_eksw, order.main_color_eksw_id]
        ...
        if (order.persida != nil)
          ...
          csv << ["ARTPOS", main_index, index.to_s, 1, "m2", @persida.sungate_code, @persida_color.sungate_code, 
            order.persida, order.pl_persidas, order.up_persidas, "<Discount/Surcharge%>", "<Discount/Surcharge per unit>", order.timi_m_persidas, "no", order.price_persidas, market_price_persidas.round(2), dealer_price_persidas.round(2), order.price_color_persidas.round(2)]
            index +=10
        end
        #prostasia
        if (order.prostasia_name != nil)
        ...
        if (order.rolo != nil && order.timi_m_epik != nil)
          ...
          csv << ["ARTPOS", main_index, index.to_s, 1, "m2", @rolo_epik.sungate_code, @rolo_color.sungate_code, 
            order.rolo, order.pl_rolou_ep, order.up_rolou_ep, "<Discount/Surcharge%>", "<Discount/Surcharge per unit>", order.timi_m_epik, "no", order.price_rolou, market_price_rolou.round(2), dealer_price_rolou.round(2), order.price_color_rolou.round(2)]
          index +=10
        end
        #rolo eks
        if (order.rolo != nil && order.timi_m_ekso != nil)
          ...
          csv << ["ARTPOS", main_index, index.to_s, 1, "m2", @rolo_eks.sungate_code, @rolo_color.sungate_code, 
            order.rolo, order.pl_rolou_ek, order.up_rolou_ek, "<Discount/Surcharge%>", "<Discount/Surcharge per unit>", order.timi_m_ekso, "no", order.price_rolou, market_price_rolou.round(2), dealer_price_rolou.round(2), order.price_color_rolou.round(2)]
          index +=10
        end
        #window still
        if (order.window_still != nil)
