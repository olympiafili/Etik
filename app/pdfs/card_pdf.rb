class Card_Pdf < Prawn::Document
  ...
  def total_price
    ...
    data_price = make_table([ 
                            ["<b>#{I18n.t("translate.Starting_price")}:</b>","","","<color rgb='008000'>"+price(@sunolo)+"</color> €"],
  ...
  def pinakas_rows
  	...
  	@items.map do |order| 
  		...
  		data2 = make_table([
                  ...
                  if (!order.typos_katw_1.nil?) then  ["<b>#{order.aukson}.#{counter = counter + 1}: #{I18n.t("translate.Types")}</b>: #{order.typos_katw_1}, #{order.typos_code} \n #{I18n.t("translate.Quantity")}: #{order.typos_katw_arithmos_1} | #{I18n.t("translate.Length")}: #{order.tm_p_t1} mm ","m","<color rgb='008000'>(#{price(order.timi_m_t_1)})</color>","<color rgb='ff0000'>#{price(order.timi_typos_katw_1)}</color> €"] else [{:content => "", :height => 0},{:content => "", :height => 0}]  end,
                  ...
                  if (!order.lock.nil?) then  ["<b>#{order.aukson}.#{counter = counter + 1}: #{I18n.t("translate.HardwareAccessories")}</b>: #{order.lock}","","#{if (order.timi_m_lock?) then "<color rgb='008000'>(#{price(order.timi_m_lock)})</color>" end}"," <color rgb='ff0000'>"+price(order.price_lock)+"</color> €"] else [{:content => "", :height => 0},{:content => "", :height => 0}]  end,
                  ...
                  if (!order.handle.nil?) then  ["<b>#{order.aukson}.#{counter = counter + 1}: #{I18n.t("translate.Handles")}</b>: #{order.handle}","","#{if (order.timi_m_handle?) then "<color rgb='008000'>(#{price(order.timi_m_handle)})</color>" end}"," <color rgb='ff0000'>"+price(order.price_handle)+"</color> €"] else [{:content => "", :height => 0},{:content => "", :height => 0}]  end,
                  if (!order.figuren.nil?) then  ["<b>#{order.aukson}.#{counter = counter + 1}: #{I18n.t("translate.Figures")}</b>: #{order.figuren}","","#{if (order.timi_m_figuren?) then "<color rgb='008000'>(#{price(order.timi_m_figuren)}%)</color>" end}"," <color rgb='ff0000'>"+price(order.price_figuren)+"</color> €"] else [{:content => "", :height => 0},{:content => "", :height => 0}]  end,
                  if (!order.sprossen.nil?) then  ["<b>#{order.aukson}.#{counter = counter + 1}: #{I18n.t("translate.Sprossen")}</b>: #{order.sprossen}","","#{if (order.timi_m_sprossen?) then "<color rgb='008000'>(#{price(order.timi_m_sprossen)})</color>" end}"," <color rgb='ff0000'>"+price(order.price_sprossen)+"</color> €"] else [{:content => "", :height => 0},{:content => "", :height => 0}]  end,
                  if (!order.lufter.nil?) then  ["<b>#{order.aukson}.#{counter = counter + 1}: #{I18n.t("translate.Lufters")}</b>: #{order.lufter}","","#{if (order.timi_m_lufter?) then "<color rgb='008000'>(#{price(order.timi_m_lufter)})</color>" end}"," <color rgb='ff0000'>"+price(order.price_lufter)+"</color> €"] else [{:content => "", :height => 0},{:content => "", :height => 0}]  end,
                  if (!order.kopp.nil?) then  ["<b>#{order.aukson}.#{counter = counter + 1}: #{I18n.t("translate.Kopplungen")} #{I18n.t("translate.Left")}</b>: #{order.kopp}","","#{if (order.timi_m_kopp?) then "<color rgb='008000'>(#{price(order.timi_m_kopp)})</color>" end}"," <color rgb='ff0000'>"+price(order.price_kopp)+"</color> €"] else [{:content => "", :height => 0},{:content => "", :height => 0}]  end,
                  if (!order.kopp_de.nil?) then  ["<b>#{order.aukson}.#{counter = counter + 1}: #{I18n.t("translate.Kopplungen")} #{I18n.t("translate.Right")}</b>: #{order.kopp_de}","","#{if (order.timi_m_kopp_de?) then "<color rgb='008000'>(#{price(order.timi_m_kopp_de)})</color>" end}"," <color rgb='ff0000'>"+price(order.price_kopp_de)+"</color> €"] else [{:content => "", :height => 0},{:content => "", :height => 0}]  end,

                  if (!order.kopp_pa.nil?) then  ["<b>#{order.aukson}.#{counter = counter + 1}: #{I18n.t("translate.Kopplungen")} #{I18n.t("translate.Top")}</b>: #{order.kopp_pa}","","#{if (order.timi_m_kopp_pa?) then "<color rgb='008000'>(#{price(order.timi_m_kopp_pa)})</color>" end}"," <color rgb='ff0000'>"+price(order.price_kopp_pa)+"</color> €"] else [{:content => "", :height => 0},{:content => "", :height => 0}]  end,
                  if (!order.kopp_ka.nil?) then  ["<b>#{order.aukson}.#{counter = counter + 1}: #{I18n.t("translate.Kopplungen")} #{I18n.t("translate.Bottom")}</b>: #{order.kopp_ka}","","#{if (order.timi_m_kopp_ka?) then "<color rgb='008000'>(#{price(order.timi_m_kopp_ka)})</color>" end}"," <color rgb='ff0000'>"+price(order.price_kopp_ka)+"</color> €"] else [{:content => "", :height => 0},{:content => "", :height => 0}]  end,

                  #if (!order.equipment.nil?) then
                    #order.equipment.zip(order.price_equipment).each do |equip, price_equip|
                      #["<b>#{order.aukson}.#{counter = counter + 1}: #{I18n.t("translate.Equipment")}</b>: #{equip}","#{if (price_equip) then "<color rgb='008000'>(#{price(price_equip)})</color>" end}"," <color rgb='ff0000'>"+price(price_equip)+"</color> €"] 
                    #end
                  #else
